...
from agents.ada_agent.opponent_model import OpponentModel
from agents.ada_agent.acceptance_strategy import AcceptanceStrategy
from agents.ada_agent.bidding_strategy import BiddingStrategy
from agents.ada_agent.learning_model import LearningModel


class ADAgent(DefaultParty):
    """
        Template Agent

        Copy the 'ada_agent' directory and change the name of it.
    """
    NAME: str = "Template Agent"

    def __init__(self):
        super(ADAgent, self).__init__()
    ...
    def notifyChange(self, data: Inform):
        ...
        # that is performed by one of the agents (including yourself).
            ...
            if actor != self.me:
                ...
                self.receive_action(action, reward=0.0, learning_rate=0.0, discount_factor=0.0)
    ...
    def receive_action(self, action: Action, reward: float, learning_rate: float, discount_factor: float):
        """
            This method will be called when an Action received.
        :param action: Action can be Offer or Accept
        :param reward: Reward value
        :param learning_rate: Learning rate value
        :param discount_factor: Discount factor value
        :return: None
        """
        if isinstance(action, Offer):
            ...
            self.opponent_model.update(bid, self.last_received_bid, reward, learning_rate, discount_factor)
            # update bidding strategy with bid
            self.bidding_strategy.receive_bid(bid)
            # update learn model with bid
            if self.learning_model is not None:
                self.learning_model.receive_bid(bid, self.last_received_bid, reward, learning_rate, discount_factor)
            ...
            self.learning_model.reach_agreement(self.last_generated_bid, True, reward, learning_rate, discount_factor)
